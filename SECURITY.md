...
kCTF exposes infrastructure in which authors are able to expose CTF challenges with security vulnerabilities (including SQL injection vulnerabilities). Even with code execution, it should be impossible for an attacker to savotage or gain an advantage over other players. In order to accomplish this, the challenge provides tools to sandbox and isolate every instance of a challenge from other users.
